...
## 开源版/上线版对比
| 功能 | [开源版](https://github.com/flyhunterl/glucose-buddy) | 上线版 |
|---|---|---|
| 多用户账号 | ❌ 单实例、单用户使用 | ✅ 支持注册、登录和多用户同时使用 |
| 用户数据隔离 | ❌ 一个实例共用一套数据 | ✅ 每个用户独立保存血糖、餐食、运动、消息等数据 |
| 登录方式 | ⚠️ 仅支持单一访问密码 | ✅ 本地账号、Linux.do 登录、邮箱验证和密码找回 |
| CGM 数据接入 | ⚠️ 主要从已有 Nightscout 拉取 | ✅ 每个用户使用独立 API Secret 直接上传 |
| 个体化血糖预测 | ❌ 使用通用趋势外推算法，不训练个人模型 | ✅ 根据每个用户的历史数据训练个人机器学习模型 |
| 预测时间 | ✅ 预测未来 10、20、30 分钟，不显示个人模型区间 | ✅ 预测未来 10、20、30 分钟，并显示预测区间 |
| 餐食和运动感知预测 | ⚠️ 只进行规则型生活方式修正 | ✅ 将碳水、蛋白质、脂肪、胰岛素和运动加入模型 |
| 个人敏感度数据库 | ❌ 不支持 | ✅ 记录并计算个人餐食、运动敏感度 |
| 具体食物敏感度 | ❌ 不支持 | ✅ 可分析不同食物对个人血糖的影响 |
| 具体运动敏感度 | ❌ 不支持 | ✅ 可分析不同运动对个人血糖的影响 |
| 网页录入餐食 | ❌ 主要读取 Nightscout 已有记录 | ✅ 支持，并可记录三大营养素和胰岛素 |
| AI 营养估算 | ❌ 不支持 | ✅ 输入食物后自动估算碳水、蛋白质和脂肪 |
| 网页录入运动 | ❌ 主要读取 Nightscout 已有记录 | ✅ 支持运动类型、时长和备注 |
| 网页录入指尖血糖 | ❌ 主要读取 Nightscout 已有记录 | ✅ 支持 |
| 修改记录 | ❌ 不支持站内修改 | ✅ 支持修改当天的餐食、运动和指尖血糖记录 |
| 分类历史查询 | ❌ 没有独立分类查询页面 | ✅ 可分别查询餐食、运动和指尖血糖历史 |
| AI 使用方式 | ⚠️ 实例统一配置 AI，没有用户次数管理 | ✅ 内置 AI 次数制，也可配置免费自定义 AI |
| 后台低血糖报警 | ⚠️ 单实例统一报警 | ✅ 每个用户可独立开启，网页关闭后仍定时检查 |
| 报警通知 | ⚠️ 使用实例统一配置的收件人和通知密钥 | ✅ 每用户独立 XXTUI，邮件发送到用户已验证邮箱 |
| 数据保留 | ✅ 没有内置的 90 天用户数据限制 | ⚠️ 普通用户保留 90 天，高级用户长期保留 |
| 治疗方案配置 | ✅ 可配置药物和胰岛素方案 | ⏳ 后期补充 |
